...
#pragma warning disable 649
        ...
        public static string UnityDllsDirPath;
...
#pragma warning restore 649
        ...
        static JsonDocument LoadJson()
        {
            string jsonPath = Path.Combine(
                CommonPath.Unity.AssetsDirPath,
                Path.Combine("NativeScript", "NativeScriptTypes.json"));
            string json = File.ReadAllText(jsonPath);
            return Newtonsoft.Json.JsonConvert.DeserializeObject<JsonDocument>(json);
        }

        static Assembly[] GetAssemblies(string[] assemblyNames)
        {
            const int numDefaultAssemblies = 3;
            ...
            else
            {
                ...
                for (int i = 0; i < assemblyNames.Length; ++i)
                {
                    string path = assemblyNames[i]
                        .Replace("UNITY_PROJECT_LIB", CommonPath.Unity.ProjectDllDirPath)
                        .Replace("UNITY_PROJECT", CommonPath.Unity.ProjectDirPath)
                        .Replace("UNITY_ASSETS", CommonPath.Unity.AssetsDirPath)
                        .Replace("DOTNET_DLLS", CommonPath.DotNetDllsDirPath)
            ...
            StringBuilders builders)
        {
            ...
            csharpContents = StringUtilities.InjectIntoString(
                csharpContents,
            ...
            cppSourceContents = StringUtilities.InjectIntoString(
                cppSourceContents,
                "/*BEGIN FUNCTION POINTERS*/",
                "\n\t/*END FUNCTION POINTERS*/",
                builders.CppFunctionPointers.ToString());
            cppHeaderContents = StringUtilities.InjectIntoString(
                cppHeaderContents,
            ...
            String[] files = { CsharpPath, CppHeaderPath, CppSourcePath };
            String[] content = { csharpContents, cppHeaderContents, cppSourceContents };

            for (int i = 0; i < 3; i++)
            {
                var permissionSet = new PermissionSet(PermissionState.None);
                var writePermission = new FileIOPermission(FileIOPermissionAccess.Write, files[i]);
                permissionSet.AddPermission(writePermission);

                if (permissionSet.IsSubsetOf(AppDomain.CurrentDomain.PermissionSet))
                {
                    File.WriteAllText(files[i], content[i]);
                    Debug.Log("Bindings written to " + files[i]);
                }
                else
                {
                    Debug.LogError("No write permission for the file " + files[i]);
                }
            }

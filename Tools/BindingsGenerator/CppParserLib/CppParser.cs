/* CppHeader2CS was created by Ryan White  (Updated: 2/23/2017)
   ...
   Modified for this project by Giang Tong (Jan 2021)
*/
...
namespace CppParser
{
    public class Parser
    {
        ...
        public static SortedDictionary<string, string> typeConversions = new SortedDictionary<string, string>()
        {
            ...
            // STL
            { "std::uint8_t", "System.Byte" },
            { "std::uint16_t", "System.UInt16" },
            { "std::uint32_t", "System.UInt32" },
            { "std::int8_t", "System.SByte" },
            { "std::int16_t", "System.Int16" },
            { "std::int32_t", "System.Int32" },

            // Custom
            { "Vector3","Vector3"},
        };

        public static ParseResult ParseFileName(string fullFilePath)
        {
            ...
            {
                Console.Write("Error: Error reading " + fullFilePath + ".  Details:" + ex.ToString());
                Console.WriteLine("usage: cppHeaderParse.exe input_file [output_file]");
            }

            ParseResult result = ParseFile(fileContent);
            result.fullFilePath = fullFilePath;
            result.fileName = Path.GetFileName(fullFilePath);
            return result;
        }

        private static ParseResult ParseFile(string fileContent)
        {
            ParseResult parseResult = new ParseResult();
            parseResult.rawText = fileContent;
            ...
            try
            {
                MatchCollection matches = Regex.Matches(fileContent, mainParser, RegexOptions.Multiline |
                ...
                foreach (Match match in matches)
                {
                    ...
                    else if (match.Groups["struct_name"].Success)
                    {
                        ...
                        cppStruct.name = match.Groups["struct_name"].Value;
                        ...
                        foreach (Match structRow in structRows)
                            if (!structRow.ToString().Contains("C2CS_SKIP"))
                            {
                                ...
                                cppStruct.members.Add(convertedVar);
                        ...
                        parseResult.structs.Add(cppStruct);
                    }
